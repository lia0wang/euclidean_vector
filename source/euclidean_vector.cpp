...
namespace comp6771 {
	...
	auto euclidean_vector::operator+=(euclidean_vector const& v2) -> euclidean_vector&
	{
		...
		std::transform(magnitude_.get(),
					   ...
					   v2.magnitude_.get(),
					   magnitude_.get(), std::plus<double>());
		e_norm_ = -1;
		return *this;
	}
	...
	auto euclidean_vector::operator-=(euclidean_vector const& v2) -> euclidean_vector&
	{
		...
		std::transform(magnitude_.get(),
		               magnitude_.get() + dimension_,
					   v2.magnitude_.get(),
					   magnitude_.get(), std::minus<double>());
		e_norm_ = -1;
		return *this;
	}
	...
	auto euclidean_vector::operator*=(double v2) noexcept -> euclidean_vector&
	{
		std::transform(magnitude_.get(),
					   magnitude_.get() + dimension_,
					   magnitude_.get(),
					   [&](double x) { return std::multiplies<double>()(x, v2); });
		e_norm_ = -1;
		return *this;
	}
	...
	auto euclidean_vector::operator/=(double v2) -> euclidean_vector&
	{
		...
		std::transform(magnitude_.get(),
					   magnitude_.get() + dimension_,
					   magnitude_.get(),
					   [&](double x) { return std::divides<double>()(x, v2); });
		e_norm_ = -1;
		return *this;
	}
	...
	{
		return euclidean_vector::is_dimension_equal(v1, v2) and
	...
	auto dot(euclidean_vector const& v1, euclidean_vector const& v2) -> double
	{
		...
		return v1.dimensions() == 0 ? 0 : std::inner_product(v1.magnitude_.get(),
															 v1.magnitude_.get() + v1.dimension_,
															 v2.magnitude_.get(),
														     0.0);
	}
	...
	auto euclidean_vector::is_dimension_equal(euclidean_vector const& v1,
									                    ...
									                    euclidean_vector const& v2) -> void
	{
		if (not euclidean_vector::is_dimension_equal(v1, v2))
			throw euclidean_vector_error("Dimensions of LHS(" + std::to_string(v1.dimension_)
										 + ") and RHS(" + std::to_string(v2.dimension_)
										 + ") do not match");
	}
